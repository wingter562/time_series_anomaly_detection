...
def fit(data_file='data_std.txt', config='parameters.yaml', slotting=True, plotting=False):
    """
    ...
    :param data_file: contains data to fit
    ...
    if data_file == '':  # use file path specified in config if not given as an argument
    ...
    start_hour_shift = 0
    training_data_size_limit = params['training_data_size_limit']
    ...
    train_data = common_funcs.read_data(data_file,
                                  skips=headlines,
                                  cols=tuple(range(num_channels + 1)),
                                  datatype=JZLogFrame_type)
    start_date = train_data[start_hour_shift][0]
    train_data = train_data[start_hour_shift:training_data_size_limit]
    ...
    glob_decisions_map = list(range(start_hour_shift, training_data_size_limit + start_hour_shift))
    model_set = {}
    ...
    decisions_with_stamps = np.array([train_data[start_hour_shift:start_hour_shift+training_data_size_limit],
                                      glob_decisions_map]).T
    common_funcs.save_data(decisions_save_path, decisions_with_stamps, linenum=False)

    # plot decisions on training data
    if plotting:
        plot.scatter(range(start_hour_shift, training_data_size_limit), glob_decisions_map, s=1 ** 2)
        plot.hlines(y=0, xmin=start_hour_shift, xmax=training_data_size_limit, linestyles='dashed')
...
def detect(data_file='', config='parameters.yaml', model_file='saved_model.yaml', plotting=True):
    """
    # Detect anomalies in the specified data from a file using pre-trained models
    :param data_file: data file containing test data in the format like:
    ...
    # read data from the cleaned, normalized/standardized data set
    test_data = common_funcs.read_data(data_file,
                                       skips=headlines,
                                       cols=tuple(range(num_channels + 1)),
                                       datatype=JZLogFrame_type)
    test_data_size = len(test_data)
    start_date = test_data[0][0]
    end_date = test_data[test_data_size-1][0]
    ...
    glob_decisions_map = list(range(0, test_data_size))  # decisions map
    for slot in slots:
        time_seq = np.array(slot)[:, 0].tolist()  # get timestamp sequence and transform it to hour-index sequence
        for k in range(len(time_seq)):
            time_seq[k] = common_funcs.count_hours_from_str(time_seq[k])  # convert stamp to absolute time
    ...
    # plot results
    if plotting:
        plot.scatter(range(0, test_data_size), glob_decisions_map, s=1 ** 2)
        plot.hlines(y=0, xmin=0, xmax=test_data_size, linestyles='dashed')
        plot.title(model_name + "\ndecision map (+1/-1 denotes normality/anomaly)")
        plot.text(0, -0.5, start_date, rotation=90)
        plot.text(test_data_size, -0.5, end_date, rotation=90)
        plot.show()
...
#detect(data_file='data_std.txt',config='parameters.yaml',model_file='saved_model.yaml',plotting=True)
