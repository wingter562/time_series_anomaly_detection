...
def fit(train_file='data_std.txt', config='parameters.yaml', model_file='', slotting=True, plotting=False):
    ...
    # print(train_data)
    print("INFO@OCSVM: Number of slots = %d" % num_models)
    print("INFO@OCSVM: Model training in each slot starts...")
    ...
    for slot in slots:
        ...
        OCSVM_model = OneClassSVM(kernel=SVMKernel, gamma=gamma, nu=contamination)
        OCSVM_model.fit(np.delete(np.array(slot), 0, 1))  # feed timestamp-stripped slot data

        # cache each slot-wise model
        model_set.append(OCSVM_model)
        print("INFO@OCSVM: model id%s cached." % id(OCSVM_model))
        model_id += 1
    ...
    print("INFO@OCSVM: Dumping models into %s" % model_save_path)
    with open(model_save_path, 'wb') as msp:
        pickle.dump(model_set, msp)

    # save training data decisions to disk
    print("INFO@OCSVM: Dumping decisions of training data into %s" % decisions_save_path)
    ...
    if plotting:
        plot.scatter(range(training_data_range_limit[0], training_data_range_limit[1]), glob_decisions_map,
                     s=1 ** 2)
...
def detect(test_file='', config='parameters.yaml', model_file='saved_model.mdl', plotting=True):
    ...
    SVMKernel = params['SVMKernel']
    gamma = params['gamma']
    contamination: 0.1 = params['contamination']
    decisions_save_path = params['decision_save_path']
    model_save_path = params['model_save_path']
    ...
    print("INFO@OCSVM: loading model(s)...")
    with open(model_file, 'rb') as mdl:
        models = pickle.load(mdl)
    print("INFO@OCSVM: %d model(s) are loaded into memory..." % len(models))
    ...
    for slot in slots:
        ...
        ocsvm = models[slot_id]

        # make decisions for test data in this slot
        local_decisions_map = ocsvm.decision_function(np.delete(np.array(slot), 0, 1)).tolist()  # timestamp stripped
    ...
    print("INFO@OCSVM: detection finished, decisions returned...")
